...
class TestEnvironment : public ::testing::Environment {
 public:
  ...
  virtual void SetUp()
  {
    ...
    if (mysql_query(my_con, "DROP TABLE IF EXISTS truncation"))
        FAIL() << "Could not drop existing table: " << mysql_error(my_con);
    if (mysql_query(my_con, "CREATE TABLE IF NOT EXISTS truncation (a varchar(10)) engine=columnstore"))
...
/* Test if chained error on truncation works */
TEST(Truncation, Truncation)
{
    std::string table("truncation");
    ...
    if (mysql_query(my_con, "DROP TABLE truncation"))
