...
class TestEnvironment : public ::testing::Environment {
 public:
  ...
  virtual void SetUp()
  {
    ...
    if (mysql_query(my_con, "DROP TABLE IF EXISTS dataconvertint"))
        FAIL() << "Could not drop existing table: " << mysql_error(my_con);
    if (mysql_query(my_con, "CREATE TABLE IF NOT EXISTS dataconvertint (a int, b double, c varchar(50)) engine=columnstore"))
...
TEST(DataConvertInt, DataConvertInt)
{
    std::string table("dataconvertint");
    ...
    try {
        ...
        bulk->setColumn(1, (uint64_t) 1);
        bulk->setColumn(2, (uint64_t) 1);
        bulk->writeRow();
        bulk->setColumn(0, (uint64_t) 2147483647);
        bulk->setColumn(1, (uint64_t) 2147483647);
        bulk->setColumn(2, (uint64_t) 2147483647);
        bulk->writeRow();
        bulk->commit();
    } catch (mcsapi::ColumnStoreError &e) {
        FAIL() << "Error caught: " << e.what() << std::endl;
    }
    if (mysql_query(my_con, "SELECT * FROM dataconvertint"))
    ...
    if (mysql_query(my_con, "DROP TABLE dataconvertint"))
