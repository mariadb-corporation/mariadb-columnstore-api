...
class TestEnvironment : public ::testing::Environment {
 public:
  ...
  virtual void SetUp()
  {
    ...
    if (mysql_query(my_con, "DROP TABLE IF EXISTS mcol1792"))
        FAIL() << "Could not drop existing table: " << mysql_error(my_con);
    if (mysql_query(my_con, "CREATE TABLE IF NOT EXISTS mcol1792 (a int, b datetime(6)) engine=columnstore"))
  ...
  virtual void TearDown()
  {
    if (my_con)
    {
        if (mysql_query(my_con, "DROP TABLE mcol1792"))
...
/* Make sure datetime decimal results are correct */
TEST(mcol1792, mcol1792)
{
    std::string table("mcol1792");
    ...
    try {
        ...
        bulk->setColumn(0, 1);
        bulk->setColumn(1, "2018-01-01 00:00:01.220");
        bulk->writeRow();
        bulk->commit();
    } catch (mcsapi::ColumnStoreError &e) {
        FAIL() << "Error caught: " << e.what() << std::endl;
    }
    if (mysql_query(my_con, "SELECT * FROM mcol1792"))
    ...
    ASSERT_STREQ(row[1], "2018-01-01 00:00:01.220000");
